...
def get_chunks(dataset_dir):
    ...
    chunksize = 1000000
    chunks = pd.read_csv(dataset_dir, chunksize=chunksize, header=None, compression='gzip',
                         sep='\t', error_bad_lines=False, low_memory=False, quoting=3)
...
def get_dataframe(title, language="enwiki", month="2021-01"):
    ...
    data = []
    chunks = get_chunks(DUMP_DIR + CLICKSTREAM_FN)

    for chunk in chunks:
        # Memory Optimization
        # Changing the datatype of "Type" attribute to category
        chunk[2] = chunk[2].astype('category')
        chunk = chunk[(chunk[0].str.title() == title.title()) |
                      (chunk[1].str.title() == title.title())]
        data.append(chunk)
